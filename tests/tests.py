...
# Intented to be rewritten from scratch
import os
import smtk_draco

#import numpy as np

testdata_directory = "resources"
...
def test_decoding_and_encoding_mesh_file():
    decoder = smtk_draco.Decoder()

    with open(os.path.join(testdata_directory, "bunny.drc"), "rb") as draco_file:
        assert decoder.decode(draco_file.read()), "Expected the file to be decoded"

    assert decoder.get_vertex_count() == EXPECTED_POINTS_BUNNY_MESH
    assert decoder.get_index_count() / 3  == EXPECTED_FACES_BUNNY
    # TODO: add way to check attribute count (in this test: normals: 0)
    ...
    assert decoder.get_vertex_count() == EXPECTED_POINTS_BUNNY_MESH
    assert decoder.get_index_count() / 3 == EXPECTED_FACES_BUNNY
    # TODO: add way to check attribute count (in this test: normals = EXPECTED_POINTS_BUNNY_MESH)
